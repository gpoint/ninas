<template>
  <div>
    ...
    >
      ...
      <optgroup
        v-if="optionsType === 'groups'"
        v-for="group in options"
        :key="group"
        :label="group.label"
      >
        <option v-for="opt in group.options" :key="opt" :value="opt.value">
          {{ opt.text }}
        </option>
      </optgroup>
    </select>
...
export default {
  ...
  inherit: false,
  template:
    "<select class='form-control' v-model='model' :name='name' style='width: 100%'>" +
